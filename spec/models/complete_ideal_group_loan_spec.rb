...
describe GroupLoan do
  ...
  context "start doing the weekly transaction. basic payment all the way" do 
    ...
    it "should do the realistic case #1: unpaid backlogs during the grace period, but cleared all" do
      #CONDITION: financial education + loan disbursement is done. weekly payment cycle starts
      ...
      closing_result.should be_nil # because loan default has to be proposed and executed
      ...
      @group_loan.reload
      @group_loan.propose_default_payment_execution( @field_worker ) # cashier is notified
      @group_loan.is_default_payment_resolution_proposed.should be_false #because we need to approve all pending transactions in grace period
      @group_loan.reload
      @group_loan.execute_default_payment_execution( @cashier ) 
      ...
      @group_loan.active_group_loan_memberships.order("created_at ASC").each do |glm|
        ...
        total_actual_compulsory_savings.should == expected
        extra_savings_before_group_loan_close_array <<  glm.member.saving_book.total_extra_savings 
        expected_difference_in_extra_savings_after_group_loan_close_array << expected  # because compulsory savings will be migrated to voluntary savings
      end
      ...
      @group_loan.close_group_loan(@branch_manager)
      # on  group loan close: migrate the $$$ from compulsory savings associated with this group loan 
      @group_loan.is_closed.should be_false  # because all payment in grace payment has to be approved
      # @group_loan.reload 
      
      # approve all the pending transaction
      @group_loan.pending_approval_grace_period_transactions.each do |transaction_activity|
