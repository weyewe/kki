  ...
  context 'should ensure reversability between backlog payment and future payment' do
    before(:each) do 
      ...
      @transaction_activity = TransactionActivity.create_generic_independent_payment(
              ...
              @field_worker,
              @independent_cash, 
              @independent_savings_withdrawal,
              @independent_number_of_weeks,
              @independent_number_of_backlogs,
              false)
      @transaction_activity.should be_valid 
    end
    
    it 'should give the correct post condition after weekly task closing' do
      @weekly_task.is_weekly_attendance_marking_done.should be_true 
      @weekly_task.is_weekly_payment_collection_finalized.should be_true 
      @weekly_task.is_weekly_payment_approved_by_cashier.should be_true 
      @first_glm.reload 
      ...
      # the independent payment must clear the backlog 
      
    
    end
    
    it 'should create revert the cleared backlog payment if   the independent payment is updated'  do
      
      ActiveRecord::Base.transaction do
        new_number_of_weeks = 1
        new_number_of_backlogs = 0 
        @new_transaction_activity = TransactionActivity.update_generic_independent_payment(
                @first_glm,
                @field_worker,
                @independent_cash, 
                @independent_savings_withdrawal,
