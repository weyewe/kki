...
describe GroupLoan do
  ...
  context "do payment of 1 week, introduce update. can it be updated?" do 
    before(:each) do
      ...
      @first_glm = @group_loan.active_group_loan_memberships.order("created_at DESC")[1]
      @second_glm = @group_loan.active_group_loan_memberships.order("created_at DESC")[2]
    ...
    it 'should be in grace period' do 
      @group_loan.is_grace_period?.should be_true 
    end
    
    it 'should not allow non defaultee to create grace payment ' do
    ...
    it 'should allow defaultee to create grace payment' do 
      puts "213-- The unpaid backlogs of @first_glm: #{@first_glm.unpaid_backlogs.count }"
      transaction_activity = TransactionActivity.create_generic_grace_period_payment(
              @first_glm,
              @field_worker,
              BigDecimal('1000'),
      ...
      # first_update = nil 
      # ActiveRecord::Base.transaction do
      #   first_update = TransactionActivity.update_generic_grace_period_payment(
      #           @first_glm,
      #           @field_worker,
      #           BigDecimal('0'),
      #           BigDecimal('0') )
      # end
      #         
      # first_update.should be_nil 
      # yeah, it does the rollback 
      ...
      first_update.should be_valid 
      
      another_transaction = TransactionActivity.create_generic_grace_period_payment(
              @first_glm,
              @field_worker,
              BigDecimal('1000'),
              BigDecimal('0'),
      ...
      another_transaction.should be_nil
    end
